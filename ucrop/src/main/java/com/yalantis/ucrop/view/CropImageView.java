...
public class CropImageView extends TransformImageView {
    ...
    private InitImageSidesListener mInitImageSidesListener;
    ...
    public SaveBitmapUseCase createSaveBitmapUseCase(Uri outputUri) {
        ...
        return new SaveBitmapUseCase.Builder(getContext())
                .setBitmap(getViewBitmap())
                .setOutputUri(outputUri)
                .setCropRect(mCropRect)
                .setImageRect(RectUtils.trapToRect(mCurrentImageCorners))
                .setAngle(getCurrentAngle())
                .setScale(getCurrentScale())
                .setMaxResultImageSizeX(mMaxResultImageSizeX)
                .setMaxResultImageSizeY(mMaxResultImageSizeY)
                .build();
    }
    ...
    public void setInitImageSidesListener(@Nullable InitImageSidesListener listener) {
        mInitImageSidesListener = listener;
    }
    ...
    protected void onImageLaidOut() {
        ...
        // can extract current image corners

        if (mInitImageSidesListener != null) {
            float w = mCurrentImageCorners[2] - mCurrentImageCorners[0];
            float h = mCurrentImageCorners[5] - mCurrentImageCorners[1];
            mInitImageSidesListener.onInitImageSides(w, h);
            mInitImageSidesListener = null;
        }
